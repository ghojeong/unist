...
int isPower2(int x) {
    ...
    int isNotZero = !!x;
    int isPower2 = ! ( ( x&(~x+1) ) ^ x );
    return isPositive & isNotZero & isPower2 ;
}
...
int rempwr2(int x, int n) {
	//TODO
    int check_n_zero = (!n) + (~0);
    int checkPositive =  (x>>31)<<n ;
    int remainder = ~( (~0)<<n ) & x;
    int check_remainder_zero = (!remainder) + (~0);
    return check_n_zero & check_remainder_zero & ( checkPositive | remainder ) ;
}
